...
class ODEBlock(nn.Module):
    ...
    def forward(self, x):
        time_end = random.uniform(0.5, 1.5)
        self.integration_time = torch.tensor([0, time_end]).float()

        self.integration_time = self.integration_time.type_as(x)
        out = odeint(self.odefunc, x, self.integration_time, rtol=args.tol, atol=args.tol)

        # num = random.random()
        # magnitude = random.randint(-1,6)
        # tol = num * 10 **(-magnitude)
        # print(tol)
...
if __name__ == '__main__':
    ...
    for batch_idx, (data, target) in enumerate(test_loader):
        ...
        save_path = os.path.join(r'output/result_pic_ode_random', method_name)
        tot += 1
        ...
        # 这里指定用什么方法去攻击。
        attack = foolbox.attacks.DeepFoolL2Attack()
